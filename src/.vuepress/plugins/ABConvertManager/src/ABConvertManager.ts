...
// AB转换器容器
import {ABConvert, ABConvert_IOEnum, type ABConvert_IOType} from './converter/ABConvert'
        ...
        for (let item_header of list_header) { // TODO 因为可能被插入新的“中间自动转换器”，要么for替换成递归，要么都在头部预处理时弄完
            for (let abReplaceProcessor of ABConvertManager.getInstance().list_abConvert) {
                ...
                if (typeof (abReplaceProcessor.match) == 'string') {
                    if (abReplaceProcessor.match != item_header) continue
                } else {
                    if (!abReplaceProcessor.match.test(item_header)) continue
                }
                // TODO 删除旧的别名系统
                // 检查是否有别名。若是，递归
                ...
                // 若不是，使用process方法
                    ...
                    if (abReplaceProcessor.process_param != prev.prev_type2) {
                        ...
                        if (abReplaceProcessor.process_param == ABConvert_IOEnum.el &&
                            ...
                            const subEl: HTMLDivElement = document.createElement("div");
